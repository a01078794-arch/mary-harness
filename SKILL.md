...
3. **누적 카운터 갱신.** FAILLOG 상단 표를 매 작업 갱신한다.
   ...
   **`실행된 하네스 작업`은 `task_id` 기준으로 한 번만 센다.** `paused`였다가 재개해 끝난 작업은
   재개 횟수만큼 세지 않는다 — 그러지 않으면 분모가 부풀어 모든 비율이 낮게 보인다.
   종료 상태가 바뀌면(예: `paused` → `completed`) 이전 상태의 카운터를 **빼고** 새 상태에 더한다.
...
|---|---|---|---|
| `completed` | 완료 조건 충족 + 4-1/4-4 검증 통과 + 결과 인계 | **삭제** | 검증 완료 +1 |
...
카운터는 `task_id` 기준이다 — 상태가 바뀌면 이전 칸에서 빼고 새 칸에 더한다. 중복 가산하지 않는다.
