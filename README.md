...
> ⚠ **rv.1 — 실사용 검증 전.** 절차 설계까지만 끝났고 실측 데이터가 없습니다.
> 프로덕션 신뢰 금지.
...
```bash
# macOS / Linux
mkdir -p ~/.claude/skills/LLM && cp SKILL.md LAYERS.md ~/.claude/skills/LLM/
```
```powershell
# Windows (PowerShell)
New-Item -ItemType Directory -Force ~/.claude/skills/LLM
Copy-Item SKILL.md, LAYERS.md ~/.claude/skills/LLM/
```
...
`~/.claude/LLM/`에 자동으로 만들어지고, OS·셸 같은 환경 정보는 Claude가 직접 확인해 채웁니다.

## 언제 발동하나

`/LLM` 호출, 또는 요청이 아래 셋 중 하나일 때만.
...
2. **설정 없이 돌아간다.** 위치를 묻지 않고, 탐색하지 않고, 항상 `~/.claude/LLM/`에 쓴다.
...
내 컴퓨터에 자동으로 생기는 것 — `~/.claude/LLM/` 안. **저장소에 올라가지 않습니다.**
...
`~/.claude/LLM`은 Windows·macOS·Linux에서 모두 그 기기의 홈 아래 같은 자리를 가리킵니다
(`C:\Users\<이름>\.claude\LLM` / `/Users/<이름>/.claude/LLM` / `/home/<이름>/.claude/LLM`).
기기마다 따로 설정할 것이 없습니다.
...
```
- 2026-07-22 | L17/cache-staleness | scope: web-research
...
- [ ] 실제 작업 5~10건 돌려보고 단계별 마찰 지점 기록
- [ ] 2단계(후보 3개)가 실제로 유용한지 — **3번째 후보가 한 번도 채택되지 않으면 제거 신호**
- [ ] 반례를 축소한 경우와 서브에이전트를 돌린 경우의 지적 품질 비교
- [ ] 승격 기준(2회)이 적절한지 실측
- [ ] 빈 상태에서 설치 → 첫 실행 → `~/.claude/LLM/` 자동 생성이 Windows·macOS 양쪽에서 되는지
